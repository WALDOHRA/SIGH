...
Begin VB.UserControl ucInsumoYcpt 
   ...
   Begin UltraGrid.SSUltraGrid grdInsumos 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      Appearance      =   "ucInsumoYcpt.ctx":0000
      CaptionAppearance=   "ucInsumoYcpt.ctx":003C
      Caption         =   "Insumos"
   End
   ...
   Begin VB.Label Label1 
      Caption         =   "Teclas ayuda: <F10>=Agreg.CPT     <Supr> = Eliminar    <F11>=Agreg.Insumo"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   8.25
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "ucInsumoYcpt"
...
'        Programa: Control para CPT e Insumos de Imágenes
...
Dim ml_MontoDerechoInforme As Double
...
Property Let MontoDerechoInforme(lValue As Double)
    ml_MontoDerechoInforme = lValue
End Property
Property Get MontoDerechoInforme() As Double
    MontoDerechoInforme = ml_MontoDerechoInforme
End Property
...
Sub Inicializar()
    ...
    lnMaximaCantidadExamen = Val(lcBuscaParametro.SeleccionaFilaParametro(294))   'debb-06-03-2012
End Sub
...
Sub CargaProductosPorIdMovimiento()
    ...
    CargarItemsALaGrillaS rs
    Set rs = mo_ReglasImagenes.ImagMovimientoCPTSeleccionarPorIdMovimiento(ml_idMovimiento)
    CargarItemsALaGrillaCPT rs, False
    On Error Resume Next
    mrs_FacturacionProductos.MoveFirst
    grdProductos_Click
...
Sub CargarItemsALaGrillaS(rs As Recordset)
    ...
    Do While Not rs.EOF
            ...
            If ml_NoPermiteCargarCantidadFallada = False Then
               mrs_FacturacionInsumos!cantidadFallada = rs!cantidadFallada
            End If
...
Sub CargarItemsALaGrillaCPT(rs As Recordset, lbSeCargaDesdeBoletaOpcionAgregar As Boolean)
    ...
    lbPrimeraVez = True
    Do While Not rs.EOF
        If Val(lcBuscaParametro.SeleccionaFilaParametro(250)) = rs.Fields!idProductoCpt Then
           ml_MontoDerechoInforme = rs!Importe
        Else
...
Dim dTotalPagado As Double
    ...
    dTotalPagado = ml_MontoDerechoInforme
...
Dim dTotalAnulado As Double
    ...
    dTotalExonerado = 0
    dTotalPagoACuenta = 0
    dTotalIngresado = ml_MontoDerechoInforme
...
Sub OnKeyDown(oGrilla As SSUltraGrid, KeyCode As UltraGrid.SSReturnShort)
     If ml_PermiteAgregarItems = True Or lbEstoyEnGridCPT = False Then
    ...
    End If
        ...
        Case vbKeyF11
            ...
            'grdInsumosFocusColumna "NombreProducto"
            grdInsumos.SetFocus
            AgregaProductoInsumo
            If optPorCodigo.Value = True Then
                grdInsumosFocusColumna "codigo"
            Else
                grdInsumosFocusColumna "NombreProducto"
            End If
        End Select
           ...
           ' End If
            ...
            If oGrilla.ActiveCell.Column.Key = "NombreProducto" Then
                ...
                Case Else
                    ...
                    If lbEstoyEnGridCPT = True Then
                       grillaBusqueda.Top = grdInsumos.Top - 1000   'oGrilla.ActiveCell.GetUIElement.Rect.Bottom * Screen.TwipsPerPixelY
                    Else
...
Private Sub InicializarLaGrilla(oGrilla As SSUltraGrid)
    ...
    Else
       ...
       oGrilla.Bands(0).Columns("Cantidad").Header.Caption = "Cant.Imag.Tomadas"
       oGrilla.Bands(0).Columns("Cantidad").Width = 1800
    End If
    ...
    End If
    If lbEstoyEnGridCPT = False Then
...
Private Sub UserControl_Resize()
   ...
   grdProductos.Width = UserControl.Width - 80    '1800
   grdProductos.Height = 2000 '2550
   grdInsumos.Top = grdProductos.Top + grdProductos.Height + 30
   grdInsumos.Left = 10
   grdInsumos.Width = UserControl.Width - 80
   ...
   optPorCodigo.Top = UserControl.Height - UserControl.Label1.Height - 20
   optPorDescripcion.Top = UserControl.Height - UserControl.Label1.Height - 20
...
Sub grdInsumosFocusColumna(lcNombreColumna As String)
...
Public Sub habilitar(lbHabilitar As Boolean)
    grdProductos.Enabled = lbHabilitar
    grdInsumos.Enabled = lbHabilitar
End Sub
