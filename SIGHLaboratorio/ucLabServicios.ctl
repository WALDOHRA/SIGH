...
Begin VB.UserControl ucLabServicios 
   ClientHeight    =   5730
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   11685
   ...
   End
End
Attribute VB_Name = "ucLabServicios"
...
'        Programa: Control para Servicios
...
Sub CargarItemsALaGrillaS(rs As Recordset)
  ...
  Do While Not rs.EOF
    ...
    mrs_FacturacionProductos!TotalPorPagar = mrs_FacturacionProductos!Cantidad * mrs_FacturacionProductos!PrecioUnitario
    mrs_FacturacionProductos!SeUsaSinPrecio = rsCatHos!SeUsaSinPrecio
    rs.MoveNext
  Loop
  mb_CargandoProductos = False
...
Sub ConfigurarProductoPorCodigo(oGrilla As SSUltraGrid)
  ...
  Dim oRow As SSRow
    ...
    Select Case ms_TipoProducto
    Case sghServicio
        'Set rs = mo_AdminCaja.SeleccionarServiciosPorNombreOCodigoSegunTipofinanciamientoYpuntoCarga(sghPorCodigo, oRow.Cells("codigo").Value, oRow.Cells("idtipofinanciamiento").Value, ml_IdPuntoCarga, sghInsumosYcpt)
    ...
    If rs.RecordCount > 0 Then
            ...
            If Not ItemYaExiste(rs.Fields("idproducto").Value) Then
                ...
                oRow.Cells("SeUsaSinPrecio").Value = rsCatHos.Fields("SeUsaSinPrecio").Value
            End If
    End If
    Set rsCatHos = Nothing
End Sub
...
Private Sub InicializarLaGrilla(oGrilla As SSUltraGrid)
    ...
    If ml_PermiteVerColumnaCantidadFallada = True Then
       oGrilla.Bands(0).Columns("NombreProducto").Width = 5000
    Else
       oGrilla.Bands(0).Columns("NombreProducto").Width = 5000
    End If
...
'    oGrilla.Bands(0).Columns("IdTipoFinanciamiento").Style = ssStyleDropDownList
    ...
    oGrilla.Bands(0).Columns("IdEstadoFacturacion").Width = 1000
    ...
    oGrilla.Bands(0).Columns("idPuntoCarga").Width = 2000
    ...
    Else
       oGrilla.Bands(0).Columns("preciounitario").Activation = ssActivationAllowEdit 'ssActivationActivateNoEdit
    End If
    If ml_PermiteVerColumnaCantidadFallada = True Then
