...
Begin VB.UserControl ucFacturacionItems 
   ...
   Begin VB.Menu mnuProductos 
      ...
      End
      Begin VB.Menu mnuGastosMedicos 
         Caption         =   "Gastos Médicos"
      End
   ...
   End
End
Attribute VB_Name = "ucFacturacionItems"
...
Dim lnIdGastosMedicos As Long
Dim lnIdMedicamentos As Long
...
Sub Inicializar()
    ...
    lnIdGastosMedicos = Val(lcBuscaParametro.SeleccionaFilaParametro(568))
    lnIdMedicamentos = Val(lcBuscaParametro.SeleccionaFilaParametro(569))
...
Sub CargaProductosPorIdReceta(rs As Recordset)
    ...
    End If
End Sub
Sub CargaProductosDeLaBoleta(rs As Recordset)
    Dim rsCatHos As New Recordset, lcMensaje As String, lbContinar As Boolean
    mb_CargandoProductos = True
    lcMensaje = ""
    Do While Not rs.EOF
...
Dim oRow As SSRow
    ...
    If rs.RecordCount > 0 Then
       'debb-14022011
       If rs.Fields("idproducto").Value <> lnIdPagosACuenta And rs.Fields("idproducto").Value <> lnIdOtrosAdm And _
          rs!idProducto <> lnIdGastosMedicos And rs!idProducto <> lnIdMedicamentos Then
       'debb-14022011
...
'''    End With
'''    'mgaray201410f
'''    Set mrs_FacturacionProductosSinDatos = mrs_FacturacionProductos.Clone()
'''    'Set grdProductos.DataSource = mrs_FacturacionProductos
...
Private Sub InicializarLaGrilla(oGrilla As SSUltraGrid)
    ...
    oGrilla.Bands(0).Columns("NombreProducto").Width = 8200   '9000
...
Private Sub mnuGastosMedicos_Click()
    AgregaLineaCPT 2
End Sub
...
Sub AgregaLineaCPT(lnOpcionDelMenu As Integer)
    AgregaPagoBanco lnOpcionDelMenu
    grdProductos.Bands(0).Columns("preciounitario").Activation = ssActivationAllowEdit
    grdProductos.Bands(0).Columns("cantidad").Activation = ssActivationActivateNoEdit

End Sub

Sub AgregaPagoBanco(lnOpcionDelMenu As Integer)
    Dim lcCodigoPrd As String, lcNombrePrd As String
    Select Case lnOpcionDelMenu
    Case 1    'mnuCtaBancarias
         DevuelveCodigoYdescripcionSegunId lnIdDepositoGarantia, lcCodigoPrd, lcNombrePrd
    Case 2    'mnuGastosMedicos
         DevuelveCodigoYdescripcionSegunId lnIdGastosMedicos, lcCodigoPrd, lcNombrePrd
    Case 3    'mnuMedicamentos
         DevuelveCodigoYdescripcionSegunId lnIdMedicamentos, lcCodigoPrd, lcNombrePrd
    End Select
    mb_CargandoProductos = True
    With mrs_FacturacionProductos
        .AddNew
        .Fields!IdFacturacionProducto = 0
        .Fields!idProducto = IIf(lnOpcionDelMenu = 1, lnIdDepositoGarantia, IIf(lnOpcionDelMenu = 2, lnIdGastosMedicos, lnIdMedicamentos))
        .Fields!Codigo = lcCodigoPrd
        .Fields!NombreProducto = lcNombrePrd
        .Fields!Cantidad = 1
        .Fields!PrecioUnitario = 1
