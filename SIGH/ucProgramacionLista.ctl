...
Begin VB.UserControl ucProgramacionLista 
   ClientHeight    =   8460
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   13425
   ...
   End
End
Attribute VB_Name = "ucProgramacionLista"
...
Private Sub cmbEspecialidad_KeyDown(KeyCode As Integer, Shift As Integer)
    mo_Teclado.RealizarNavegacion KeyCode, cmbEspecialidad
    RaiseEvent SePresionoTeclaEspecial(KeyCode)
End Sub
...
Sub RefrescarListaMedicos()
    ...
    'mgaray20141014
'    Set lstMedicos.RowSource = mo_AdminProgramacionMedica.MedicosFiltrarPorDptoYEspecialidadsql2000(Val(mo_cmbDepartamento.BoundText), Val(mo_cmbEspecialidad.BoundText))
    Set lstMedicos.RowSource = mo_AdminProgramacionMedica.MedicosFiltrarPorDptoYEspecialidadConEspecialidad(Val(mo_cmbDepartamento.BoundText), Val(mo_cmbEspecialidad.BoundText))
    lstMedicos.Tag = ""
...
Dim oProgDetalle As New ProgramacionDetalle
    'franklin 2017
    ...
    oProgDetalle.IdDepartamento = Val(mo_cmbDepartamento.BoundText)
...
Dim lcSql As String, lcFecha As String
        ...
        Set oProgramaciones = mo_AdminProgramacionMedica.ProgramacionMedicaLeerPorMedicoYMes(lIDMedico, iMes, iAnio)
        If oProgramaciones.Count > 0 Then
            ...
            For Each oProgramacion In oProgramaciones
                ...
                Set oRsTmp = mo_AdminProgramacionMedica.ProgramacionMedicaXmedicoFechaHOra(Val(lstMedicos.BoundText), lcFecha, oProgramacion.HoraInicio)
                lcNombreServicio = ""
                If oRsTmp.RecordCount > 0 Then
                   lcNombreServicio = IIf(IsNull(oRsTmp.Fields!Nombre), "", oRsTmp.Fields!Nombre)
                   If lcNombreServicio = "" Then
                      lcNombreServicio = IIf(IsNull(oRsTmp.Fields!dturno), "", oRsTmp.Fields!dturno)
                   Else
                      lcNombreServicio = lcNombreServicio & " (" & oRsTmp!TipoServicio & ")"
                   End If
                End If
                oRsTmp.Close
                Set oDOTurno = mo_AdminProgramacionMedica.TurnosSeleccionarPorId(oProgramacion.IdTurno)
                Set programacion = Diario.AppointmentSet.Add(lcNombreServicio + " - " + oProgramacion.Descripcion, oProgramacion.fecha + iHoraIni / 24, oProgramacion.fecha + iHoraFin / 24)
                programacion.DataVariant = oProgramacion
                programacion.BackColor = oProgramacion.Color
            ...
            If dCantidadHorasMes > lnMaximaHorasProgramadasXmedico Then
                MsgBox "La cantidad de horas programadas al médico: " & Chr(13) & lstMedicos.Text & " es " & dCantidadHorasMes & Chr(13) & " lo cual excede las " & Trim(Str(lnMaximaHorasProgramadasXmedico)) & " horas mensuales", vbExclamation, "Programación médica"
            End If
