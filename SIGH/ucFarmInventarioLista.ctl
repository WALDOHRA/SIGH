...
Begin VB.UserControl ucFarmInventarioLista 
   ClientHeight    =   6240
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   10095
   ...
   End
End
Attribute VB_Name = "ucFarmInventarioLista"
...
'        Programa: Control para buscar Inventario
...
Public Sub RealizarBusqueda()
    Set grdLista.DataSource = mo_ReglasFarmacia.FarmInventarioDevuelveLista
...
Private Sub grdLista_AfterRowActivate()
    ...
    ml_idRegistroSeleccionado = rsRecordset("idInventario")
End Sub
...
Private Sub grdLista_InitializeLayout(ByVal Context As UltraGrid.Constants_Context, ByVal Layout As UltraGrid.SSLayout)
    grdLista.Bands(0).Columns("idInventario").Hidden = True
    grdLista.Bands(0).Columns("IdEstadoInventario").Hidden = True
    grdLista.Bands(0).Columns("numeroInventario").Header.Caption = "N° Inventario"
    grdLista.Bands(0).Columns("numeroInventario").Width = 1000
    grdLista.Bands(0).Columns("Descripcion").Header.Caption = "Almacén"
    grdLista.Bands(0).Columns("Descripcion").Width = 8000
    grdLista.Bands(0).Columns("FechaCreacion").Header.Caption = "F.Creación"
    grdLista.Bands(0).Columns("FechaCreacion").Width = 1500
...
Private Sub grdLista_InitializeRow(ByVal Context As UltraGrid.Constants_Context, ByVal Row As UltraGrid.SSRow, ByVal ReInitialize As Boolean)
        If Val(Row.Cells("IdEstadoInventario").GetText()) = 0 Then
