...
Begin VB.UserControl ucIntervencionS 
   ClientHeight    =   5685
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   13050
   ...
   Begin VB.ComboBox cmbOrden 
      ...
      EndProperty
      Height          =   315
      ItemData        =   "ucIntervencionS.ctx":0000
      Left            =   6240
      List            =   "ucIntervencionS.ctx":000A
   ...
   Begin UltraGrid.SSUltraGrid grillaBusqueda 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      Appearance      =   "ucIntervencionS.ctx":0026
      Caption         =   ".."
   End
   ...
   End
End
Attribute VB_Name = "ucIntervencionS"
...
'        Programa:  Control para Items de Intervención Sanitaria
...
Dim ml_TipoSOPORTE As String
Dim wxParametro347 As String

Property Let Parametro347(lValue As String)
   wxParametro347 = lValue
End Property

Property Let TipoSOPORTE(lValue As String)
   ml_TipoSOPORTE = lValue
End Property
...
Sub CargarItemsALaGrilla(rs As Recordset)
    ...
    Dim lnCantidad As Long, lnCantidad1 As Long
    ...
    Do While Not rs.EOF
        ...
        oRsTmp.Filter = "idTipoSalidaBienInsumo=" & sghTipoSalidaItemFarmacia.sghSoloEstrategico
        If oRsTmp.RecordCount > 0 Then
           lnCantidad1 = oRsTmp.Fields!cantidad
        End If
        '
        ...
        mRs_Productos!saldo = lnCantidad1 + lnCantidad
        oRsTmp.Close
    Loop
    mb_CargandoProductos = False
    Totalizar
...
Dim lbContinuar As Boolean
    ...
    Set rs = mo_ReglasFarmacia.farmDevuelveSaldosSinLotesSegunAlmacenSOPORTE(ml_IdAlmacen, 0, lcFiltro)
    rs.Filter = "idTipoSalidaBienInsumoSaldo=" & sghTipoSalidaItemFarmacia.sghSoloEstrategico & _
                IIf(ml_TipoSOPORTE <> "", " and TipoProductoSismed='" & ml_TipoSOPORTE & "'", "")
    ...
    If rs.RecordCount > 0 Then
        ...
        If wxParametro347 <> "S" Then
            If rs.Fields!idTipoSalidaBienInsumo = sghTipoSalidaItemFarmacia.sghSoloVenta Then
                MsgBox "Ese Medicamento es usado sólo para VENTAS"
...
Sub OnKeyPress(oGrilla As SSUltraGrid, KeyAscii As UltraGrid.SSReturnShort)
        ...
        If oGrilla.ActiveCell.Column.Key = "NombreProducto" Then
            ...
            Case Else
                ...
                Set rs = mo_ReglasFarmacia.farmDevuelveSaldosSinLotesSegunAlmacenSOPORTE(ml_IdAlmacen, 1, sNombre)
                If wxParametro347 <> "S" Then
                     rs.Filter = "idTipoSalidaBienInsumoSaldo=" & sghTipoSalidaItemFarmacia.sghSoloEstrategico & _
                                 IIf(ml_TipoSOPORTE <> "", " and TipoProductoSismed='" & ml_TipoSOPORTE & "'", "")
                Else
                     If ml_TipoSOPORTE <> "" Then
                        rs.Filter = "TipoProductoSismed='" & ml_TipoSOPORTE & "'"
                     Else
                        rs.Filter = "TipoProductoSismed<>'_'"
                     End If
                End If
...
Sub RefrescarDatos()
    ...
    If Not grillaBusqueda.ActiveRow Is Nothing Then
                ...
                If wxParametro347 <> "S" Then
                    If grillaBusqueda.ActiveRow.Cells("idTipoSalidaBienInsumo") = sghTipoSalidaItemFarmacia.sghSoloVenta Then
                        MsgBox "Ese Medicamento es sólo para VENTAS"
                        Exit Sub
                    End If
                End If
...
Public Sub cargaPaqueteElegido(lnIdPaquete As Long)
        ...
        lcMensaje = ""
        Do While Not rs.EOF
            If rs.Fields!idTipoSalidaBienInsumo = sghTipoSalidaItemFarmacia.sghSoloEstrategico Or rs.Fields!idTipoSalidaBienInsumo = sghTipoSalidaItemFarmacia.sghVentaEstrategico Then
                lnPrecio = mo_ReglasFarmacia.DevuelvePrecioSegunTipoConcepto(rs.Fields("idproducto").Value, ml_idTipoPrecioParaNiNs)
                lnCantidadSaldo = 0
                Set oRsTmp = mo_ReglasFarmacia.farmDevuelveSaldosSegunAlmacenProducto(ml_IdAlmacen, rs!idProducto)
                If oRsTmp.RecordCount > 0 Then
                   oRsTmp.MoveFirst
                   Do While Not oRsTmp.EOF
                      If oRsTmp.Fields!idTipoSalidaBienInsumo = sghTipoSalidaItemFarmacia.sghSoloEstrategico Or oRsTmp.Fields!idTipoSalidaBienInsumo = sghTipoSalidaItemFarmacia.sghVentaEstrategico Then
                ...
                End If
            Else
                lcMensaje = lcMensaje & "(Se Despacha por VENTA) " & rs!codigo & "-" & rs!Nombre & Chr(13)
            End If
            rs.MoveNext
        Loop
        mb_CargandoProductos = False
