...
Begin VB.UserControl ucFacturacionFarm 
   ClientHeight    =   5685
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   13050
   ...
   Begin UltraGrid.SSUltraGrid grillaBusqueda 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      Appearance      =   "ucFActuracionFarm.ctx":0000
      Caption         =   ".."
   End
   ...
   End
End
Attribute VB_Name = "ucFacturacionFarm"
    ...
    If rs.RecordCount > 0 Then
        ...
        Else
            ...
            If Not ItemYaExiste(rs.Fields("idproducto").Value) Then
                ...
                grdProductos.ActiveRow.Cells("IdViaAdministracion").Value = mo_reglasComunes.RecetaDetalleUltimaViaAdministracion(rs.Fields("idproducto").Value)
            End If
        End If
    End If
    oConexion.Close
...
'               End If
        ...
        If oGrilla.ActiveCell.Column.Key = "idDosisRecetada" Then
           If KeyAscii = 13 Then
              SendKeys "{Tab}"
           End If
           Exit Sub
        End If
        If oGrilla.ActiveCell.Column.Key = "IdViaAdministracion" Then
           If KeyAscii = 13 Then
              SendKeys "{Tab}"
           End If
           Exit Sub
        End If
        If oGrilla.ActiveCell.Column.Key = "Observaciones" Then
           If KeyAscii = 13 Then
               AgregaProducto (False)
               If optPorCodigo.Value = True Then
                   grdProductosFocusColumna "codigo"
...
Sub GenerarRecordsetProductos()
    With mRs_Productos
          ...
          .Fields.Append "IdViaAdministracion", adInteger
...
Private Sub InicializarLaGrilla(oGrilla As SSUltraGrid)
     ...
     Dim lnItem As Integer, lnIdDosisDefault As Integer
     grdProductos.Font.Name = "Arial narrow"
     grdProductos.Font.Size = "8"
     ...
     grdProductos.Bands(0).Columns("saldo").Hidden = True
     grdProductos.Bands(0).Columns("precio").Hidden = True
     grdProductos.Bands(0).Columns("total").Hidden = True
     grdProductos.Bands(0).Columns("nro").Hidden = True
     
     grdProductos.Bands(0).Columns("idDosisRecetada").Width = 700
     grdProductos.Bands(0).Columns("idDosisRecetada").Header.Caption = "N° Dosis"
     Set oRsTmp1 = mo_reglasComunes.RecetaDosisSelecionarTodos
     lnItem = 0
     With grdProductos.ValueLists.Add("Dosis").ValueListItems
     ...
     End With
     oRsTmp1.Close
     grdProductos.Bands(0).Columns("idDosisRecetada").ValueList = "Dosis"
     
     Set oRsTmp1 = mo_reglasComunes.RecetasListadoViasAdministracion
     With grdProductos.ValueLists.Add("ViaAdministracion").ValueListItems
        If oRsTmp1.RecordCount > 0 Then
            oRsTmp1.MoveFirst
     ...
     grdProductos.Bands(0).Columns("IdViaAdministracion").Width = 1600
     grdProductos.Bands(0).Columns("IdViaAdministracion").Header.Caption = "Via"
     
     grdProductos.Bands(0).Columns("Observaciones").Width = 1500
     grdProductos.Bands(0).Columns("Observaciones").Header.Caption = "Frecuencia"
     ...
     grdProductos.Bands(0).Columns("NombreProducto").Width = 5000
    ...
    If (Not grillaBusqueda.ActiveRow Is Nothing) Then
       If (grillaBusqueda.ActiveRow.Cells("saldo").Value > 0) Then
                ...
                If lbContinua Then
                    ...
                    grdProductos.ActiveRow.Cells("IdViaAdministracion").Value = mo_reglasComunes.RecetaDetalleUltimaViaAdministracion(grillaBusqueda.ActiveRow.Cells("idproducto").Value)
