...
Sub Form_Load()
       ...
       If lbSoloCambiaHoraFinal = True And mi_Opcion = sghModificar Then
            ...
            mo_Formulario.HabilitarDeshabilitar cmbIdServicio, False
            mo_Formulario.HabilitarDeshabilitar cmbIdTipoProgramacion, False
...
Dim sMensaje As String
   ...
   ValidarDatosObligatorios = False
    ...
    If mo_cmbIdTipoServicio.BoundText = "" Then
        sMensaje = sMensaje + "Elija el tipo de servicio" + Chr(13)
    Else
...
Function ValidarReglas() As Boolean
   ...
   If mi_Opcion <> sghAgregar Then
       If -DateDiff("h", CDate(Date & " " & Me.txtHoraInicio), CDate(Date & " " & txtHoraFin)) > 0 Then
        ...
        End If
    End If
    Select Case Val(mo_cmbIdTipoServicio.BoundText)
    Case sghTipoServicio.sghConsultaExterna, sghTipoServicio.sghEmergenciaConsultorios
            ...
            If mi_Opcion = sghAgregar Then
                ...
                If oRsTmp.RecordCount > 0 Then
                    ms_MensajeError = "El CONSULTORIO elegido NO SE PUEDE PROGRAMAR, porque es un CONSULTORIO QUE SE ATIENDE EN FORMA SIMULTANEA en: " & _
                           ...
                           "verifique en opción GENERAL->SERVICIOS->CONSULTORIOS EXTERNOS->" & oRsTmp!Nombre
            ...
            End If
     Case Else
            'Hospitalizacion/otros
            sMensaje = HayCruceDeProgramacionesDeConsultaExterna()
            If sMensaje <> "" Then
                 Dim oMensaje221 As New SIGHNegocios.clMensaje
                 oMensaje221.MostrarFormulario "Hay un cruces con la(s) siguiente(s) programacion(es) " & Chr(13) & sMensaje, Me.Caption
                 Set oMensaje221 = Nothing
    ...
    Do While daDiaSeleccionado <> 0
        ...
        Do While Not programacion Is Nothing
            ...
            'Verifica si la nueva porg es de CE
             If Val(Me.cmbIdTipoServicio) = sghTipoServicio.sghConsultaExterna Then
                If Me.IdProgramacion <> oDOProgramacion.IdProgramacion Then
                    ...
                    If oDOProgramacion.idTipoServicio = Val(mo_cmbIdTipoServicio.BoundText) Then
                        ...
                        End If
                    Else
                        If oDOProgramacion.fecha = daDiaSeleccionado And oDOProgramacion.idMedico = Me.idMedico Then
                            'Las validaciones se hacen solo si la hora inicio nueva es menor que la hora de fin existente
                            If ConvertirAMinutos(Me.txtHoraInicio) < ConvertirAMinutos(oDOProgramacion.HoraFin) Then
                                If (ConvertirAMinutos(Me.txtHoraInicio) <= ConvertirAMinutos(oDOProgramacion.HoraInicio) And ConvertirAMinutos(Me.txtHoraFin) >= ConvertirAMinutos(oDOProgramacion.HoraInicio)) Or _
                                    (ConvertirAMinutos(Me.txtHoraInicio) >= ConvertirAMinutos(oDOProgramacion.HoraInicio) And ConvertirAMinutos(Me.txtHoraFin) <= ConvertirAMinutos(oDOProgramacion.HoraFin)) Or _
...
Dim lnTiempoPromedioAtencion As Long, ldFechaSeleccionada As Date
    ...
    Do While daDiaSeleccionado <> 0
        ...
        If ExisteProgramacionParaEseServicioFecha(Val(mo_cmbIdServicio.BoundText), ldFechaSeleccionada, _
            ...
            doProgramacion.IdDepartamento = IIf(Me.IdDepartamento = 0, 1, Me.IdDepartamento)
...
Dim oRsTmp As New Recordset
    ...
    doProgramacion.IdEspecialidad = Val(mo_cmbIdEspecialidadMedico.BoundText)
    doProgramacion.Color = Me.lblColor.BackColor
    doProgramacion.IdDepartamento = IIf(Val(mo_cmbIdServicio.BoundText) = 0, 1, mo_ReglasFacturacion.ServiciosDevuelveIdDepartamento(Val(mo_cmbIdServicio.BoundText)))
    doProgramacion.idMedico = Me.idMedico
    doProgramacion.IdServicio = Val(mo_cmbIdServicio.BoundText)
    doProgramacion.TiempoPromedioAtencion = lnTiempoPromedioAtencion
...
Function ExisteProgramacionParaEseServicioFecha(lnIdServicio As Long, lcFecha As Date, _
                                                lcHoraInicio As String, lcHoraFin As String) As Boolean
    If Val(mo_cmbIdTipoServicio.BoundText) = sghTipoServicio.sghHospitalizacion Or Val(mo_cmbIdTipoServicio.BoundText) = sghTipoServicio.sghEmergenciaConsultorios Then
       Exit Function
    End If
    Dim oRsTmp As New Recordset
    Dim oMensaje2 As New SIGHNegocios.clMensaje
    ...
    Set oRsTmp = mo_AdminProgMedica.ProgramacionMedicaSeleccionarPorIdServicio(lnIdServicio)
    If oRsTmp.RecordCount > 0 Then
       oRsTmp.MoveFirst
       Do While Not oRsTmp.EOF
